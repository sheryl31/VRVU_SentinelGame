  ...
  private bool SetPointerTarget(Vector3 target, bool interactive) {
    ...
    if (interactive) {
      ...
      ReticleOuterAngle = RETICLE_MIN_OUTER_ANGLE + RETICLE_GROWTH_ANGLE;
			...
			Color[] allColors = GetComponent<MeshFilter>().mesh.colors;

			StartCoroutine(loadSegment(allColors));

			//Renderer rendererComponent = GetComponent<Renderer>();
			//rendererComponent.material.color = new Color(0,0,0,1);
	...
	private IEnumerator loadSegment(Color[] allColors) {

		for(int i=0; i < allColors.Length; i++) {
			yield return new WaitForSeconds(0.01f);

			allColors[i] = new Color(0,0,0,1);
			//yield return new WaitForSeconds(1.0f);

		}
		GetComponent<MeshFilter>().mesh.colors = allColors;
	}
